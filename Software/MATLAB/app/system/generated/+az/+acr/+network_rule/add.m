classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            ...
            this.BaseCmd = 'az acr network-rule add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az acr network-rule add -n MyRegistry --subnet /subscriptions/00000000-0000-0000-0000-000000')
            fprintf('%s\n', '        000000/resourceGroups/myRG/providers/Microsoft.Network/virtualNetworks/myvnet/subnets/mysubn')
            fprintf('%s\n', '        et')
            fprintf('%s\n', '    Add a rule to allow access for a specific IP address-range.')
            fprintf('%s\n', '        az acr network-rule add -n MyRegistry --ip-address 23.45.1.0/24')
            fprintf('%s\n', 'For more specific examples, use: az find "az acr network-rule add"')
