classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az acr taskrun list : List the taskruns for an Azure Container Registry.
            % Command group 'acr taskrun' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az acr taskrun list ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    List taskruns and show the results in a table.')
            fprintf('%s\n', '        az acr taskrun list -r MyRegistry -g MyResourceGroup -o table')
            fprintf('%s\n', 'For more specific examples, use: az find "az acr taskrun list"')
