classdef assign < azure.AzureBase
    ...
    methods
        function this = assign()
            ...
            this.BaseCmd = 'az acr task identity assign ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Enable the system-assigned identity on an existing task. This will replace all existing user-')
            fprintf('%s\n', '    assigned identities for that task.')
            fprintf('%s\n', '        az acr task identity assign -n MyTask -r MyRegistry')
            fprintf('%s\n', '    Assign user-assigned managed identities to an existing task. This will remove the existing')
            ...
            fprintf('%s\n', '        az acr task identity assign -n MyTask -r MyRegistry \')
            fprintf('%s\n', '            --identities [system] "/subscriptions/<SUBSCRIPTON ID>/resourcegroups/<RESOURCE')
            fprintf('%s\n', '        GROUP>/providers/Microsoft.ManagedIdentity/userAssignedIdentities/myUserAssignedIdentitiy"')
            fprintf('%s\n', 'For more specific examples, use: az find "az acr task identity assign"')
