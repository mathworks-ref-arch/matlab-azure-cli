classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            ...
            this.BaseCmd = 'az acr task timer add ';
        end
        ...
        function this = timer_name(this, varargin)
            % The name of the timer trigger.
            this.Options = [this.Options, '--timer-name', varargin{:}];
        end

        function this = enabled(this, varargin)
            % Indicates whether the timer trigger is enabled.  Allowed values: false, true.  Default: 1.
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --schedule    [Required] : The schedule of the timer trigger represented as a cron expression.')
            fprintf('%s\n', '    --timer-name  [Required] : The name of the timer trigger.')
            fprintf('%s\n', '    --enabled                : Indicates whether the timer trigger is enabled.  Allowed values:')
            fprintf('%s\n', '                               false, true.  Default: 1.')
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Add a timer trigger which schedules the task at 09:30 (UTC) every day of the week from Monday')
            fprintf('%s\n', '    through Friday.')
            fprintf('%s\n', '        az acr task timer add -n taskname -r registryname --timer-name t2 --schedule "30 9 * * 1-5"')
            fprintf('%s\n', 'For more specific examples, use: az find "az acr task timer add"')
