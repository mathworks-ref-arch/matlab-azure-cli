classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            ...
            this.BaseCmd = 'az acr task credential add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    Add a registry login credential to a task using key vault secret URIs for the username and')
            fprintf('%s\n', '    password and the task system-assigned identity.')
            fprintf('%s\n', '        az acr task credential add -n taskname -r registryname --login-server myregistry.docker.io \')
            fprintf('%s\n', '            -u https://mykeyvault.vault.azure.net/secrets/secretusername -p')
            fprintf('%s\n', '        https://mykeyvault.vault.azure.net/secrets/secretpassword \')
            fprintf('%s\n', '            --use-identity [system]')
            fprintf('%s\n', '    Add a registry login credential to a task using key vault secret URIs for the username and')
            fprintf('%s\n', '    password and a task user-assigned identity given by its client id.')
            fprintf('%s\n', '        az acr task credential add -n taskname -r registryname --login-server myregistry.docker.io \')
            fprintf('%s\n', '            -u https://mykeyvault.vault.azure.net/secrets/secretusername -p')
            fprintf('%s\n', '        https://mykeyvault.vault.azure.net/secrets/secretpassword \')
            fprintf('%s\n', '            --use-identity 00000000-0000-0000-0000-000000000000')
            fprintf('%s\n', '    Add a registry login credential to a task using a plain text username and key vault secret URI')
            fprintf('%s\n', '    for the password and the task user-assigned identity given by its client id.')
            fprintf('%s\n', '        az acr task credential add -n taskname -r registryname --login-server myregistry.docker.io \')
            fprintf('%s\n', '            -u myusername -p https://mykeyvault.vault.azure.net/secrets/secretpassword \')
            fprintf('%s\n', '            --use-identity 00000000-0000-0000-0000-000000000000')
            fprintf('%s\n', '    Add a registry login credential to a task using a plain text username and key vault secret URI')
            fprintf('%s\n', '    for the password and the default managed identity for the task if one exists.')
            fprintf('%s\n', '        az acr task credential add -n taskname -r registryname --login-server myregistry.docker.io \')
            fprintf('%s\n', '            -u myusername -p https://mykeyvault.vault.azure.net/secrets/secretpassword')
            fprintf('%s\n', '    Add a registry login credential to a task that uses only the task system-assigned identity to')
            fprintf('%s\n', '    authenticate to the registry.')
            fprintf('%s\n', '        az acr task credential add -n taskname -r registryname --login-server myregistry.docker.io \')
            fprintf('%s\n', '            --use-identity [system]')
            fprintf('%s\n', 'For more specific examples, use: az find "az acr task credential add"')
