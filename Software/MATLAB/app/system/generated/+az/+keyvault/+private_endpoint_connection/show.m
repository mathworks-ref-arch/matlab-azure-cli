classdef show < azure.AzureBase
    ...
    methods
        function this = show()
            % az keyvault private-endpoint-connection show : Show details of a private endpoint connection
            % associated with a Key Vault.
            % Command group 'keyvault private-endpoint-connection' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az keyvault private-endpoint-connection show ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az keyvault private-endpoint-connection show --id "/subscriptions/00000000-0000-0000-0000-00')
            fprintf('%s\n', '        0000000000/resourceGroups/myrg/providers/Microsoft.KeyVault/vaults/mykv/privateEndpointConne')
            fprintf('%s\n', '        ctions/mykv.00000000-0000-0000-0000-000000000000"')
            fprintf('%s\n', '    Show details of a private endpoint connection request for a Key Vault by ID.')
            fprintf('%s\n', '        id = (az keyvault show -n mykv --query "privateEndpointConnections[0].id")')
            fprintf('%s\n', '        az keyvault private-endpoint-connection show --id $id')
            fprintf('%s\n', '    Show details of a private endpoint connection request for a Key Vault using vault name and')
            fprintf('%s\n', '    connection name.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az keyvault private-endpoint-connection show"')
