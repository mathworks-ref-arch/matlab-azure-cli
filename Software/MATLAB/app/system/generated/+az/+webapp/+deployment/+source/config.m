classdef config < azure.AzureBase
    ...
    methods
        function this = config()
            ...
            this.BaseCmd = 'az webapp deployment source config ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az webapp deployment source config --branch master --manual-integration --name MyWebApp')
            fprintf('%s\n', '        --repo-url https://github.com/Azure-Samples/function-image-upload-resize --resource-group')
            fprintf('%s\n', '        MyResourceGroup')
            fprintf('%s\n', 'For more specific examples, use: az find "az webapp deployment source config"')
