classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            % az webapp vnet-integration add : Add a regional virtual network integration to a webapp.
            ...
            % same resource group as the webapp will be used.
            % Command group 'webapp vnet-integration' is in preview. It may be changed/removed in a
            % future release.
            this.BaseCmd = 'az webapp vnet-integration add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az webapp vnet-integration add -g MyResourceGroup -n MyWebapp --vnet MyVnetName --subnet')
            fprintf('%s\n', '        MySubnetName -s [slot]')
            fprintf('%s\n', '    Add a regional virtual network integration to a webapp using vnet resource id')
            fprintf('%s\n', '        az webapp vnet-integration add -g MyResourceGroup -n MyWebapp --vnet ''/subscriptions/[sub')
            fprintf('%s\n', '        id]/resourceGroups/[rg]/providers/Microsoft.Network/virtualNetworks/[virtual network name]''')
            fprintf('%s\n', '        --subnet MySubnetName -s [slot]')
            fprintf('%s\n', 'For more specific examples, use: az find "az webapp vnet-integration add"')
