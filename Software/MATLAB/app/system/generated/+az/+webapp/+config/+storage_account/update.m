classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az webapp config storage-account update : Update an existing Azure storage account configuration
            % on a web app. (Linux Web Apps and Windows Containers Web Apps Only).
            % This command is in preview. It may be changed/removed in a future release.
            this.BaseCmd = 'az webapp config storage-account update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --share-name --sn         : Share name (Azure Files) or container name (Azure Blob Storage).')
            fprintf('%s\n', '    --slot -s                 : The name of the slot. Default to the productions slot if not')
            fprintf('%s\n', '                                specified.')
            fprintf('%s\n', '    --slot-setting            : Slot setting.')
            fprintf('%s\n', '    --storage-type -t         : Storage type.  Allowed values: AzureBlob, AzureFiles.')
            ...
            fprintf('%s\n', '    Update an existing Azure storage account configuration on a web app. (Linux Web Apps and Windows')
            fprintf('%s\n', '    Containers Web Apps Only) (autogenerated)')
            fprintf('%s\n', '        az webapp config storage-account update --access-key MyAccessKey --account-name MyAccount')
            fprintf('%s\n', '        --custom-id CustomId --mount-path /path/to/new/mount --name MyUniqueApp --resource-group')
            fprintf('%s\n', '        MyResourceGroup --share-name MyShare --storage-type AzureFiles')
            fprintf('%s\n', 'For more specific examples, use: az find "az webapp config storage-account update"')
