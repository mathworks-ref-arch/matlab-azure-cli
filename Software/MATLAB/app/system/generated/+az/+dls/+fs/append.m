classdef append < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = append()
            % az dls fs append : Append content to a file in a Data Lake Store account.
            % Command group 'dls' is in preview. It may be changed/removed in a future release.
            this.BaseCmd = 'az dls fs append ';
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az dls fs append"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        Command group ''dls'' is in preview. It may be changed/removed in a future release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --content [Required] : Content to be appended to the file.')
            fprintf('%s\n', '    --path    [Required] : The path in the specified Data Lake Store account where the action should')
            fprintf('%s\n', '                           take place. In the format ''/folder/file.txt'', where the first ''/'' after')
            fprintf('%s\n', '                           the DNS indicates the root of the file system.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az dls fs append"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class append 
