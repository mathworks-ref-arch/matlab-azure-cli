classdef upload < azure.AzureBase
    ...
    methods
        function this = upload()
            % az dls fs upload : Upload a file or folder to a Data Lake Store account.
            % Command group 'dls' is in preview. It may be changed/removed in a future release.
            this.BaseCmd = 'az dls fs upload ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        Command group ''dls'' is in preview. It may be changed/removed in a future release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --destination-path [Required] : The full path in the Data Lake Store filesystem to upload the')
            fprintf('%s\n', '                                    file or folder to.')
            fprintf('%s\n', '    --source-path      [Required] : The path to the file or folder to upload.')
            ...
            fprintf('%s\n', '    --thread-count                : Parallelism of the upload. Default: The number of cores in the')
            fprintf('%s\n', '                                    local machine.')
            ...
            fprintf('%s\n', '        az dls fs upload --account {account} --destination-path {destination-path} --overwrite')
            fprintf('%s\n', '        --source-path {source-path}')
            fprintf('%s\n', 'For more specific examples, use: az find "az dls fs upload"')
