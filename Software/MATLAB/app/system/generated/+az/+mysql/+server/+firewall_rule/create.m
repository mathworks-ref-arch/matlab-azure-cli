classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            ...
            this.BaseCmd = 'az mysql server firewall-rule create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az mysql server firewall-rule create -g testgroup -s testsvr -n allowip --start-ip-address')
            fprintf('%s\n', '        107.46.14.221 --end-ip-address 107.46.14.221')
            fprintf('%s\n', '    Create a firewall rule allowing connections from an IP address range.')
            fprintf('%s\n', '        az mysql server firewall-rule create -g testgroup -s testsvr -n allowiprange --start-ip-')
            fprintf('%s\n', '        address 107.46.14.0 --end-ip-address 107.46.14.221')
            fprintf('%s\n', 'For more specific examples, use: az find "az mysql server firewall-rule create"')
