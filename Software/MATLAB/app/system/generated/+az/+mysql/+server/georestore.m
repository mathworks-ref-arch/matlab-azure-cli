classdef georestore < azure.AzureBase
    ...
    methods
        function this = georestore()
            % az mysql server georestore : Geo-restore a server from backup.
            this.BaseCmd = 'az mysql server georestore ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        QL/servers/testsvr2" \')
            fprintf('%s\n', '            -l westus2')
            fprintf('%s\n', 'For more specific examples, use: az find "az mysql server georestore"')
