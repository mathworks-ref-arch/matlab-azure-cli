classdef set < azure.AzureBase
    ...
    methods
        function this = set()
            % az staticwebapp hostname set : Set given sub-domain hostname to the static app. Please configure
            % a CNAME record with your DNS provider.
            % Command group 'staticwebapp hostname' is in preview. It may be changed/removed in a
            % future release.
            this.BaseCmd = 'az staticwebapp hostname set ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Set hostname to the static app.')
            fprintf('%s\n', '        az staticwebapp hostname set -n MyStaticAppName --hostname www.example.com')
            fprintf('%s\n', 'For more specific examples, use: az find "az staticwebapp hostname set"')
