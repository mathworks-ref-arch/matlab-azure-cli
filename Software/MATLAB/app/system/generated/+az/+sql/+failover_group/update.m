classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az sql failover-group update : Updates the failover group.
            this.BaseCmd = 'az sql failover-group update ';
        end
        ...
        function this = remove_db(this, varargin)
            % List of databases to remove from Failover Group.
            this.Options = [this.Options, '--remove-db', varargin{:}];
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az sql failover-group update"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --failover-policy    : The failover policy of the Failover Group.  Allowed values: Automatic,')
            fprintf('%s\n', '                           Manual.')
            fprintf('%s\n', '    --grace-period       : Interval in hours before automatic failover is initiated if an outage')
            fprintf('%s\n', '                           occurs on the primary server. This indicates that Azure SQL Database will')
            fprintf('%s\n', '                           not initiate automatic failover before the grace period expires. Please')
            fprintf('%s\n', '                           note that failover operation with --allow-data-loss option might cause')
            fprintf('%s\n', '                           data loss due to the nature of asynchronous synchronization.')
            fprintf('%s\n', '    --remove-db          : List of databases to remove from Failover Group.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az sql failover-group update"')
