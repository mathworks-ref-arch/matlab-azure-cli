classdef set_primary < azure.AzureBase
    ...
    methods
        function this = set_primary()
            ...
            this.BaseCmd = 'az sql failover-group set-primary ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --allow-data-loss    : Complete the failover even if doing so may result in data loss. This will')
            fprintf('%s\n', '                           allow the failover to proceed even if a primary database is unavailable.')
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Set the primary of the failover group by failing over all databases from the current primary')
            fprintf('%s\n', '    server. (autogenerated)')
            fprintf('%s\n', '        az sql failover-group set-primary --name MyFailoverGroup --resource-group MyResourceGroup')
            fprintf('%s\n', '        --server myserver')
            fprintf('%s\n', 'For more specific examples, use: az find "az sql failover-group set-primary"')
