classdef import < azure.AzureBase
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az sql db import -s myserver -n mydatabase -g mygroup -p password -u login --storage-key')
            fprintf('%s\n', '        MYKEY== \')
            fprintf('%s\n', '            --storage-key-type StorageAccessKey \')
            fprintf('%s\n', '            --storage-uri https://myAccountName.blob.core.windows.net/myContainer/myBacpac.bacpac')
            fprintf('%s\n', 'For more specific examples, use: az find "az sql db import"')
