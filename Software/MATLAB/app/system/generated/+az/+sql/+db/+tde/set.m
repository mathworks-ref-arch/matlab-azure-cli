classdef set < azure.AzureBase
    ...
    methods
        function this = set()
            ...
            this.BaseCmd = 'az sql db tde set ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Sets a database''s transparent data encryption configuration. (autogenerated)')
            fprintf('%s\n', '        az sql db tde set --database mydb --resource-group MyResourceGroup --server myserver')
            fprintf('%s\n', '        --status Enabled')
            fprintf('%s\n', 'For more specific examples, use: az find "az sql db tde set"')
