classdef restart < azure.AzureBase
    ...
    methods
        function this = restart()
            % az deploymentmanager rollout restart : Restarts the rollout.
            % Command group 'deploymentmanager' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az deploymentmanager rollout restart ';
        end
        ...
        function this = skip_succeeded(this, varargin)
            % Skips all the steps that have succeeded in the previous retries of this rollout.
            this.Options = [this.Options, '--skip-succeeded', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --skip-succeeded                    : Skips all the steps that have succeeded in the previous')
            fprintf('%s\n', '                                          retries of this rollout.')
            ...
            fprintf('%s\n', '    Restart the rollout and skip all steps that have succeeded in the previous run')
            fprintf('%s\n', '        az deploymentmanager rollout restart -g rg1 -n contosoServiceRollout --skip-succeeded')
            fprintf('%s\n', 'For more specific examples, use: az find "az deploymentmanager rollout restart"')
