classdef show < azure.AzureBase
    ...
    methods
        function this = show()
            ...
            this.BaseCmd = 'az dla job recurrence show ';
        end
        function this = recurrence_identity(this, varargin)
            % Recurrence ID.
            this.Options = [this.Options, '--recurrence-identity', varargin{:}];
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az dla job recurrence show"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        Command group ''dla'' is in preview. It may be changed/removed in a future release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --recurrence-identity [Required] : Recurrence ID.')
            fprintf('%s\n', '    --end-date-time                  : The end date for when to get recurrence and aggregate its')
            fprintf('%s\n', '                                       data. The startDateTime and endDateTime can be no more than')
            fprintf('%s\n', '                                       30 days apart.')
            fprintf('%s\n', '    --start-date-time                : The start date for when to get the recurrence and aggregate')
            fprintf('%s\n', '                                       its data. The startDateTime and endDateTime can be no more')
            fprintf('%s\n', '                                       than 30 days apart.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az dla job recurrence show"')
