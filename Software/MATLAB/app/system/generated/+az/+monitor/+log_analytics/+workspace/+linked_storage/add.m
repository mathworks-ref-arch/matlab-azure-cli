classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            % az monitor log-analytics workspace linked-storage add : Add some linked storage accounts with
            % specific data source type for log analytics workspace.
            this.BaseCmd = 'az monitor log-analytics workspace linked-storage add ';
        end
        ...
        function this = storage_accounts(this, varargin)
            % List of Name or ID of Azure Storage Account.
            this.Options = [this.Options, '--storage-accounts', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --storage-accounts  [Required] : List of Name or ID of Azure Storage Account.')
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Add two linked storage accounts for a log analytics workspace')
            fprintf('%s\n', '        az monitor log-analytics workspace linked-storage add --type AzureWatson -g MyResourceGroup')
            fprintf('%s\n', '        --workspace-name MyWorkspace --storage-accounts SA1 SA2')
