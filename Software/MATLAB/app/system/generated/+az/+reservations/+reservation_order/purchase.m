classdef purchase < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = purchase()
            % az reservations reservation-order purchase : Purchase reservation order.
            ...
            % release.
            this.BaseCmd = 'az reservations reservation-order purchase ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --reservation-order-id   [Required] : Id of reservation order to purchase, generate by `az')
            ...
            fprintf('%s\n', '        az reservations reservation-order purchase --reservation-order-id {reservationOrderId} --sku')
            fprintf('%s\n', '        standard_b1ls --location westus --reserved-resource-type VirtualMachines --billing-scope')
            fprintf('%s\n', '        {SubId} --term P1Y --billing-plan Upfront --quantity 1 --applied-scope-type Single')
            fprintf('%s\n', '        --applied-scope {SubId} --display-name test')
            fprintf('%s\n', 'For more specific examples, use: az find "az reservations reservation-order purchase"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class purchase 
