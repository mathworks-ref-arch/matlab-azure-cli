classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az cosmosdb sql stored-procedure create : Create an SQL stored procedure under an Azure Cosmos
            % DB SQL container.
            this.BaseCmd = 'az cosmosdb sql stored-procedure create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --body -b           [Required] : StoredProcedure body, you can enter it as a string or as a')
            fprintf('%s\n', '                                     file, e.g., --body @sprocbody-file.json.')
            fprintf('%s\n', '    --container-name -c [Required] : Container name.')
            fprintf('%s\n', '    --database-name -d  [Required] : Database name.')
            fprintf('%s\n', '    --name -n           [Required] : StoredProcedure name.')
            ...
            fprintf('%s\n', '        az cosmosdb sql stored-procedure create -g MyResourceGroup -a MyAccount -d MyDatabase -c')
            fprintf('%s\n', '        MyContainer -n MyStoredProcedure -b StoredProcedureBody')
            fprintf('%s\n', 'For more specific examples, use: az find "az cosmosdb sql stored-procedure create"')
