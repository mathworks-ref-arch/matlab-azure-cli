classdef lock < azure.AzureBase
    ...
    methods
        function this = lock()
            % az appconfig kv lock : Lock a key-value to prohibit write operations.
            this.BaseCmd = 'az appconfig kv lock ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --label             : If no label specified, lock entry with null label. Filtering is not')
            fprintf('%s\n', '                          supported.')
            ...
            fprintf('%s\n', '        az appconfig kv lock --connection-string')
            fprintf('%s\n', '        Endpoint=https://contoso.azconfig.io;Id=xxx;Secret=xxx --key color --label test --yes')
            fprintf('%s\n', 'For more specific examples, use: az find "az appconfig kv lock"')
