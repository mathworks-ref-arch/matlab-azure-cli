classdef unlock < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = unlock()
            % az appconfig kv unlock : Unlock a key-value to gain write operations.
            this.BaseCmd = 'az appconfig kv unlock ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az appconfig kv unlock : Unlock a key-value to gain write operations.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --key    [Required] : Key to be unlocked.')
            ...
            fprintf('%s\n', '    --label             : If no label specified, unlock entry with null label. Filtering is not')
            fprintf('%s\n', '                          supported.')
            ...
            fprintf('%s\n', '        az appconfig kv unlock --connection-string')
            fprintf('%s\n', '        Endpoint=https://contoso.azconfig.io;Id=xxx;Secret=xxx --key color --label test --yes')
            fprintf('%s\n', 'For more specific examples, use: az find "az appconfig kv unlock"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class unlock 
