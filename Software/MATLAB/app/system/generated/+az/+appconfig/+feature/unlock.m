classdef unlock < azure.AzureBase
    ...
    methods
        function this = unlock()
            % az appconfig feature unlock : Unlock a feature to gain write operations.
            % Command group 'appconfig feature' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az appconfig feature unlock ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --feature [Required] : Key of the feature to be unlocked.')
            ...
            fprintf('%s\n', '    --label              : If no label specified, unlock the feature flag with null label by')
            fprintf('%s\n', '                           default.')
            ...
            fprintf('%s\n', '        az appconfig feature unlock --connection-string')
            fprintf('%s\n', '        Endpoint=https://contoso.azconfig.io;Id=xxx;Secret=xxx --feature color --label test --yes')
            fprintf('%s\n', 'For more specific examples, use: az find "az appconfig feature unlock"')
