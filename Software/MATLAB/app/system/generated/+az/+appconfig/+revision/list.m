classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az appconfig revision list : Lists revision history of key-values.
            this.BaseCmd = 'az appconfig revision list ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az appconfig revision list --connection-string')
            fprintf('%s\n', '        Endpoint=https://contoso.azconfig.io;Id=xxx;Secret=xxx --key color --datetime')
            fprintf('%s\n', '        "2019-05-01T11:24:12Z"')
            fprintf('%s\n', '    List revision history for all items and query only key, value and last_modified.')
            fprintf('%s\n', '        az appconfig revision list --connection-string')
            fprintf('%s\n', '        Endpoint=https://contoso.azconfig.io;Id=xxx;Secret=xxx --fields key value last_modified')
            fprintf('%s\n', 'For more specific examples, use: az find "az appconfig revision list"')
