classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az eventgrid domain create : Create a domain.
            this.BaseCmd = 'az eventgrid domain create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     location=<location>`.')
            fprintf('%s\n', '    --name -n           [Required] : Name of the domain.')
            ...
            fprintf('%s\n', '        az eventgrid domain create -g rg1 --name domain1 -l westus2 --input-schema customeventschema')
            ...
            fprintf('%s\n', '    Create a new domain that accepts events published in CloudEvents V1.0 schema and maps a property')
            fprintf('%s\n', '    mytopicfield to the topic name.')
            fprintf('%s\n', '        az eventgrid domain create -g rg1 --name domain1 -l westus2 --input-schema')
            fprintf('%s\n', '        cloudeventschemav1_0 --input-mapping-fields topic=mytopicfield')
            fprintf('%s\n', '    Create a new domain which allows specific inbound ip rules.')
            fprintf('%s\n', '        az eventgrid domain create -g rg1 --name domain1 -l westus2 --public-network-access enabled')
            fprintf('%s\n', '        --inbound-ip-rules 10.0.0.0/8 Allow --inbound-ip-rules 10.2.0.0/8 Allow --sku basic')
            fprintf('%s\n', 'For more specific examples, use: az find "az eventgrid domain create"')
