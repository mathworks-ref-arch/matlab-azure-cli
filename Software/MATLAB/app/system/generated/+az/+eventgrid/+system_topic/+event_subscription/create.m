classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az eventgrid system-topic event-subscription create : Create a new event subscription for a
            % system topic.
            % Command group 'eventgrid system-topic event-subscription' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az eventgrid system-topic event-subscription create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    Create a new event subscription for an Event Grid system topic, with a filter specifying a')
            fprintf('%s\n', '    subject prefix.')
            fprintf('%s\n', '        az eventgrid system-topic event-subscription create --name es4 \')
            fprintf('%s\n', '            -g rg1 --system-topic-name systemtopic1 \')
            fprintf('%s\n', '            --endpoint https://contoso.azurewebsites.net/api/f1?code=code \')
            fprintf('%s\n', '            --subject-begins-with mysubject_prefix')
            fprintf('%s\n', '    Create a new event subscription for an Event Grid system topic, using default filters, and')
            fprintf('%s\n', '    CloudEvent V 1.0 as the delivery schema.')
            fprintf('%s\n', '        az eventgrid system-topic event-subscription create -n es2 \')
            fprintf('%s\n', '            -g rg1 --system-topic-name systemtopic1 \')
            fprintf('%s\n', '            --endpoint https://contoso.azurewebsites.net/api/f1?code=code \')
            fprintf('%s\n', '            --event-delivery-schema cloudeventschemav1_0')
            fprintf('%s\n', '    Create a new event subscription for an Event Grid system topic, with a deadletter destination')
            ...
            fprintf('%s\n', '    Create a new event subscription for an Event Grid system topic, using Azure Active Directory')
            ...
            fprintf('%s\n', '    Create a new event subscription for an Event Grid system topic, using Azure Function as')
            fprintf('%s\n', '    destination.')
