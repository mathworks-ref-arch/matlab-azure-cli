classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az eventgrid system-topic event-subscription update : Update an event subscription of a system
            % topic.
            % Command group 'eventgrid system-topic event-subscription' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az eventgrid system-topic event-subscription update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '            -g rg1 --system-topic-name systemtopic1 --endpoint')
            fprintf('%s\n', '        https://contoso.azurewebsites.net/api/f1?code=code')
            fprintf('%s\n', '    Update an event subscription for an Event Grid system topic to specify a new subject-ends-with')
            fprintf('%s\n', '    filter.')
            fprintf('%s\n', '        az eventgrid system-topic event-subscription update --name es2 \')
            fprintf('%s\n', '            -g rg1 --system-topic-name systemtopic1 \')
            fprintf('%s\n', '            --subject-ends-with .jpg')
            fprintf('%s\n', '    Update an event subscription for an Event Grid system topic to specify a new endpoint and a new')
            ...
            fprintf('%s\n', '    Update an event subscription for an Azure Event Grid system topic, to include a deadletter')
            fprintf('%s\n', '    destination.')
            fprintf('%s\n', '        az eventgrid system-topic event-subscription update --name es2 \')
            fprintf('%s\n', '            -g rg1 --system-topic-name systemtopic1 \')
