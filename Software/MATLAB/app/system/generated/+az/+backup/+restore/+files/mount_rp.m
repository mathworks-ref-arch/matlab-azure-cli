classdef mount_rp < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = mount_rp()
            % az backup restore files mount-rp : Download a script which mounts files of a recovery point.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup restore files mount-rp ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az backup restore files mount-rp --container-name MyContainer --item-name MyItem --resource-')
            fprintf('%s\n', '        group MyResourceGroup --rp-name MyRp --vault-name MyVault')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup restore files mount-rp"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class mount_rp 
