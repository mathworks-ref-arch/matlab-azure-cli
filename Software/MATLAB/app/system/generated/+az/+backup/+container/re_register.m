classdef re_register < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = re_register()
            % az backup container re-register : Reset the registration details for a given container.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup container re-register ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Reset the registration details for a given container. To be used only in error scenarios as')
            fprintf('%s\n', '    specified here (https://docs.microsoft.com/azure/backup/backup-sql-server-azure-troubleshoot#re-')
            fprintf('%s\n', '    registration-failures). Understand the failure symptoms and causes before attempting re-')
            fprintf('%s\n', '    registration.')
            fprintf('%s\n', '        az backup container re-register --resource-group MyResourceGroup --vault-name MyVault')
            fprintf('%s\n', '        --container-name MyContainer --workload-type MSSQL --backup-management-type AzureWorkload')
            fprintf('%s\n', '        --yes')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup container re-register"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class re_register 
