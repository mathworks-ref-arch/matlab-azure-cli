classdef set_policy < azure.AzureBase
    ...
    methods
        function this = set_policy()
            % az backup item set-policy : Update the policy associated with this item. Use this to change
            % policies of the backup item.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup item set-policy ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --policy-name -p    [Required] : Name of the Backup policy. You can use the backup policy list')
            fprintf('%s\n', '                                     command to get the name of a backup policy.')
            ...
            fprintf('%s\n', '                                     ''Resource Id'' arguments.')
            fprintf('%s\n', '    --name -n                      : Name of the backed up item. You can use the backup item list')
            ...
            fprintf('%s\n', '        az backup item set-policy --vault-name MyVault --resource-group MyResourceGroup --container-')
            fprintf('%s\n', '        name MyContainer --name MyItem --policy-name MyPolicy')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup item set-policy"')
