classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az backup protectable-item list : Retrieve all protectable items within a certain container or
            % across all registered containers.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup protectable-item list ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    It consists of all the elements in the hierarchy of the application. Returns DBs and their upper')
            fprintf('%s\n', '    tier entities like Instance, AvailabilityGroup etc.')
            fprintf('%s\n', '        az backup protectable-item list --resource-group MyResourceGroup --vault-name MyVault')
            fprintf('%s\n', '        --workload-type MSSQL --container-name MyContainer')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup protectable-item list"')
