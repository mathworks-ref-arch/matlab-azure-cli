classdef wait < azure.AzureBase
    ...
    methods
        function this = wait()
            % az backup job wait : Wait until either the job completes or the specified timeout value is
            % reached.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup job wait ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --name -n [Required] : Name of the job. You can use the backup job list command to get the name')
            fprintf('%s\n', '                           of a job.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az backup job wait"')
