classdef update_for_vm < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = update_for_vm()
            % az backup protection update-for-vm : Update disk exclusion settings associated with a backed up
            % VM item.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup protection update-for-vm ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --disk-list-setting            : Option to decide whether to include or exclude the disk or')
            fprintf('%s\n', '                                     reset any previous settings to default behavior.  Allowed')
            fprintf('%s\n', '                                     values: exclude, include, resetexclusionsettings.')
            fprintf('%s\n', '    --diskslist                    : List of disks to be excluded or included.')
            fprintf('%s\n', '    --exclude-all-data-disks       : Option to specify to backup OS disk only.  Allowed values:')
            fprintf('%s\n', '                                     false, true.')
            ...
            fprintf('%s\n', '        az backup protection update-for-vm --vault-name MyVault --resource-group MyResourceGroup')
            fprintf('%s\n', '        --container-name MyContainer --item-name MyItem --disk-list-setting exclude --diskslist 1')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup protection update-for-vm"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class update_for_vm 
