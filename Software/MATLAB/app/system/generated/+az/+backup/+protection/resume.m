classdef resume < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = resume()
            % az backup protection resume : Resume backup for the associated backup item. Use this to change
            % the policy associated with the backup item.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup protection resume ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Resume backup for the associated backup item. Use this to change the policy associated with the')
            fprintf('%s\n', '    backup item.')
            fprintf('%s\n', '        az backup protection resume --vault-name MyVault --resource-group MyResourceGroup')
            fprintf('%s\n', '        --container-name MyContainer --item-name MyItem --policy-name MyPolicy')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup protection resume"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class resume 
