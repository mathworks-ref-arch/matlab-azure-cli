classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az backup recoverypoint list : List all recovery points of a backed up item.
            % Command group 'backup' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az backup recoverypoint list ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az backup recoverypoint list --container-name MyContainer --item-name MyItem --resource-')
            fprintf('%s\n', '        group MyResourceGroup --vault-name MyVault')
            fprintf('%s\n', 'For more specific examples, use: az find "az backup recoverypoint list"')
