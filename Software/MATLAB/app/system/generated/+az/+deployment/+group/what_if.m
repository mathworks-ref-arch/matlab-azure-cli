classdef what_if < azure.AzureBase
    ...
    methods
        function this = what_if()
            % az deployment group what-if : Execute a deployment What-If operation at resource group scope.
            % This command is in preview. It may be changed/removed in a future release.
            this.BaseCmd = 'az deployment group what-if ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        This command is in preview. It may be changed/removed in a future release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --resource-group -g      [Required] : The resource group to execute deployment What-If operation')
            fprintf('%s\n', '                                          at.')
            fprintf('%s\n', '    --aux-tenants                       : Auxiliary tenants which will be used during deployment')
            fprintf('%s\n', '                                          across tenants.')
            ...
            fprintf('%s\n', '        az deployment group what-if --resource-group testrg --name rollout01 --template-uri')
            ...
            fprintf('%s\n', '        az deployment group what-if --resource-group testrg --name rollout01 --template-uri')
            fprintf('%s\n', '        https://myresource/azuredeploy.json --parameters @myparameters.json --result-format')
            fprintf('%s\n', '        ResourceIdOnly')
            fprintf('%s\n', '    Execute a deployment What-If operation at a resource group without pretty-printing the result.')
            fprintf('%s\n', '        az deployment group what-if --resource-group testrg --name rollout01 --template-uri')
            fprintf('%s\n', '        https://myresource/azuredeploy.json --parameters @myparameters.json --no-pretty-print')
            fprintf('%s\n', 'For more specific examples, use: az find "az deployment group what-if"')
