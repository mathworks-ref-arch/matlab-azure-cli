classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az deployment mg create : Start a deployment at management group.
            this.BaseCmd = 'az deployment mg create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                                    debug logs.')
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Create a deployment at management group from a remote template file, using parameters from a')
            fprintf('%s\n', '    local JSON file.')
            fprintf('%s\n', '        az deployment mg create --management-group-id testrg --name rollout01 --location WestUS \')
            fprintf('%s\n', '            --template-uri https://myresource/azuredeploy.json --parameters @myparameters.json')
            fprintf('%s\n', '    Create a deployment at management group from a local template file, using parameters from a JSON')
            fprintf('%s\n', '    string.')
            fprintf('%s\n', '        az deployment mg create --management-group-id testmg --name rollout01 --location WestUS \')
            fprintf('%s\n', '            --template-file azuredeploy.json \')
            fprintf('%s\n', '            --parameters ''{ \"policyName\": { \"value\": \"policy2\" } }''')
            fprintf('%s\n', '    Create a deployment at management group from a local template, using a parameter file, a remote')
            fprintf('%s\n', '    parameter file, and selectively overriding key/value pairs.')
            fprintf('%s\n', '        az deployment mg create --management-group-id testmg --name rollout01 --location WestUS \')
            fprintf('%s\n', '            --template-file azuredeploy.json --parameters @params.json \')
            fprintf('%s\n', '            --parameters https://mysite/params.json --parameters MyValue=This MyArray=@array.json')
            fprintf('%s\n', 'For more specific examples, use: az find "az deployment mg create"')
