classdef exists < azure.AzureBase
    ...
    methods
        function this = exists()
            ...
            this.BaseCmd = 'az storage file exists ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az storage file exists --account-key 00000000 --account-name MyAccount --path path/file.txt')
            fprintf('%s\n', '        --share-name MyShare')
            fprintf('%s\n', '    Check for the existence of a file. (autogenerated)')
            fprintf('%s\n', '        az storage file exists --connection-string $connectionString --path path/file.txt --share-')
            fprintf('%s\n', '        name MyShare')
            fprintf('%s\n', 'For more specific examples, use: az find "az storage file exists"')
