classdef download_batch < azure.AzureBase
    ...
    methods
        function this = download_batch()
            % az storage file download-batch : Download files from an Azure Storage File Share to a local
            % directory in a batch operation.
            this.BaseCmd = 'az storage file download-batch ';
        end
        ...
        function this = validate_content(this, varargin)
            % If set, calculates an MD5 hash for each range of the file for validation. The storage service checks the hash of the content that has arrived is identical to the hash that was sent. This is mostly valuable for detecting bitflips during transfer if using HTTP instead of HTTPS. This hash is not stored. Download Control Arguments
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    directory in a batch operation.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --destination -d [Required] : The local directory where the files are downloaded to. This')
            fprintf('%s\n', '                                  directory must already exist.')
            fprintf('%s\n', '    --source -s      [Required] : The source of the file download operation. The source can be the')
            fprintf('%s\n', '                                  file share URL or the share name.')
            fprintf('%s\n', '    --dryrun                    : List the files and blobs to be downloaded. No actual data transfer')
            fprintf('%s\n', '                                  will occur.')
            ...
            fprintf('%s\n', '    (autogenerated)')
            fprintf('%s\n', '        az storage file download-batch --account-key 00000000 --account-name MyAccount --destination')
            fprintf('%s\n', '        . --no-progress --source /path/to/file')
            fprintf('%s\n', 'For more specific examples, use: az find "az storage file download-batch"')
