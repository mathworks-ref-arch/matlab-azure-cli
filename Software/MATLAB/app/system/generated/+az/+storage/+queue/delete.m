classdef delete < azure.AzureBase
    ...
    methods
        function this = delete()
            ...
            % When a queue is successfully deleted, it is immediately marked for deletion  and is no
            % longer accessible to clients. The queue is later removed from  the Queue service during
            % garbage collection.  Note that deleting a queue is likely to take at least 40 seconds to
            % complete.  If an operation is attempted against the queue while it was being deleted,  an
            % :class:`AzureConflictHttpError` will be thrown.
            this.BaseCmd = 'az storage queue delete ';
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az storage queue delete"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --fail-not-exist     : Specifies whether to throw an exception if the queue doesn''t exist.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az storage queue delete"')
