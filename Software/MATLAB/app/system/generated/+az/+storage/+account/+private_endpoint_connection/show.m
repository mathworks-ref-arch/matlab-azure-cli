classdef show < azure.AzureBase
    ...
    methods
        function this = show()
            % az storage account private-endpoint-connection show : Show details of a private endpoint
            % connection request for storage account.
            % Command group 'storage account private-endpoint-connection' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az storage account private-endpoint-connection show ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az storage account private-endpoint-connection show --id "/subscriptions/0000-0000-0000-0000')
            fprintf('%s\n', '        /resourceGroups/MyResourceGroup/providers/Microsoft.Storage/storageAccounts/mystorageaccount')
            fprintf('%s\n', '        /privateEndpointConnections/mystorageaccount.b56b5a95-0588-4f8b-b348-15db61590a6c"')
            fprintf('%s\n', '    Show details of a private endpoint connection request for storage account by ID.')
            ...
            fprintf('%s\n', '    Show details of a private endpoint connection request for storage account using account name and')
            fprintf('%s\n', '    connection name.')
            fprintf('%s\n', '        az storage account private-endpoint-connection show -g myRg --account-name mystorageaccount')
            fprintf('%s\n', '        --name myconnection')
            fprintf('%s\n', '    Show details of a private endpoint connection request for storage account using account name and')
            fprintf('%s\n', '    connection name.')
