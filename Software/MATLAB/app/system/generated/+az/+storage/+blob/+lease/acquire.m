classdef acquire < azure.AzureBase
    ...
    methods
        function this = acquire()
            % az storage blob lease acquire : Request a new lease.
            this.BaseCmd = 'az storage blob lease acquire ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     key, login.')
            fprintf('%s\n', '    --lease-duration               : Specify the duration of the lease, in seconds, or negative one')
            fprintf('%s\n', '                                     (-1) for a lease that never expires. A non-infinite lease can')
            fprintf('%s\n', '                                     be between 15 and 60 seconds. A lease duration cannot be')
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Request a new lease.')
            fprintf('%s\n', '        az storage blob lease acquire -b myblob -c mycontainer --account-name mystorageaccount')
            fprintf('%s\n', '        --account-key 0000-0000')
            fprintf('%s\n', 'For more specific examples, use: az find "az storage blob lease acquire"')
