classdef url < azure.AzureBase
    ...
    methods
        function this = url()
            ...
            this.BaseCmd = 'az storage blob url ';
        end
        ...
        function this = snapshot(this, varargin)
            % An string value that uniquely identifies the snapshot. The value ofthis query parameter indicates the snapshot version. Storage Account Arguments
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     key, login.')
            fprintf('%s\n', '    --protocol                     : Protocol to use.  Allowed values: http, https.  Default: https.')
            fprintf('%s\n', '    --snapshot                     : An string value that uniquely identifies the snapshot. The')
            fprintf('%s\n', '                                     value ofthis query parameter indicates the snapshot version.')
            ...
            fprintf('%s\n', '        az storage blob url --connection-string $connectionString --container-name container1 --name')
            fprintf('%s\n', '        blob1')
            fprintf('%s\n', '    Create the url to access a blob (autogenerated)')
            fprintf('%s\n', '        az storage blob url --account-name storageacct --account-key 00000000 --container-name')
            fprintf('%s\n', '        container1 --name blob1')
            fprintf('%s\n', 'For more specific examples, use: az find "az storage blob url"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class url 
