classdef delete < azure.AzureBase
    ...
    methods
        function this = delete()
            ...
            % The blob is marked for later deletion during garbage collection.  In order to delete a blob,
            % all of its snapshots must also be deleted. Both can be removed at the same time.
            this.BaseCmd = 'az storage blob delete ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     present, specifies the blob snapshot to delete.')
            ...
            fprintf('%s\n', '        az storage blob delete -c mycontainer -n MyBlob --account-name mystorageaccount --auth-mode')
            fprintf('%s\n', '        login')
            fprintf('%s\n', 'For more specific examples, use: az find "az storage blob delete"')
