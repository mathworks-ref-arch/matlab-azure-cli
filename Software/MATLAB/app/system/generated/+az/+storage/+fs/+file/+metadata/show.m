classdef show < azure.AzureBase
    ...
    methods
        function this = show()
            % az storage fs file metadata show : Returns all user-defined metadata, standard HTTP properties,
            % and system properties for the file.
            % It does not return the content of the file.  :keyword lease:     Required if the directory
            % or file has an active lease. Value can be a DataLakeLeaseClient object     or the lease ID
            % as a string. :type lease: ~azure.storage.filedatalake.DataLakeLeaseClient or str :keyword
            ...
            % :keyword int timeout:     The timeout parameter is expressed in seconds. :rtype:
            % FileProperties  .. admonition:: Example:      .. literalinclude::
            % ../samples/datalake_samples_upload_download.py         :start-after: [START
            % get_file_properties]         :end-before: [END get_file_properties]         :language:
            % python         :dedent: 4         :caption: Getting the properties for a file.
            this.BaseCmd = 'az storage fs file metadata show ';
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az storage fs file metadata show"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az storage fs file metadata show"')
