classdef verify < azure.AzureBase
    ...
    methods
        function this = verify()
            ...
            % Verifies a certificate by uploading a verification certificate containing the verification
            % code obtained by calling generate-verification-code. This is the last step in the proof of
            % possession process. For a detailed explanation of CA certificates in Azure IoT Hub, see
            % https://docs.microsoft.com/azure/iot-hub/iot-hub-x509ca-overview.
            this.BaseCmd = 'az iot hub certificate verify ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az iot hub certificate verify --etag AAAAAAAAAAA= --hub-name MyIotHub --name MyCertificate')
            fprintf('%s\n', '        --path /certificates/Verification.pem --resource-group MyResourceGroup --subscription')
            fprintf('%s\n', '        MySubscription')
            fprintf('%s\n', 'For more specific examples, use: az find "az iot hub certificate verify"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class verify 
