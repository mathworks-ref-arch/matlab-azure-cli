classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az policy event list : List policy events.
            this.BaseCmd = 'az policy event list ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az policy event list --resource "myKeyVault" --namespace "Microsoft.KeyVault" --resource-')
            fprintf('%s\n', '        type "vaults" -g "myresourcegroup"')
            fprintf('%s\n', '    Get policy events for a nested resource using resource name.')
            fprintf('%s\n', '        az policy event list --resource "myRule1" --namespace "Microsoft.Network" --resource-type')
            ...
            fprintf('%s\n', '    Get policy events for a policy assignment in the specified resource group in current')
            fprintf('%s\n', '    subscription.')
            fprintf('%s\n', '        az policy event list -g "myRg" -a "ddd8ef92e3714a5ea3d208c1"')
            fprintf('%s\n', '    Get top 5 policy events in current subscription, selecting a subset of properties and')
            fprintf('%s\n', '    customizing ordering.')
            fprintf('%s\n', '        az policy event list --top 5 --order-by "timestamp desc, policyAssignmentName asc" --select')
            ...
            fprintf('%s\n', '        az policy event list --filter "(policyDefinitionAction eq ''deny'' or policyDefinitionAction')
            ...
            fprintf('%s\n', '        az policy event list --apply "groupby((policyAssignmentId, policyDefinitionId,')
            ...
            fprintf('%s\n', '        az policy event list --apply "groupby((policyAssignmentId, policyDefinitionId,')
            fprintf('%s\n', '        resourceId))/groupby((policyAssignmentId, policyDefinitionId), aggregate($count as')
            fprintf('%s\n', '        numResourcesWithEvents))"')
            fprintf('%s\n', 'For more specific examples, use: az find "az policy event list"')
