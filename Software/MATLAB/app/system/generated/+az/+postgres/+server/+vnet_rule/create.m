classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az postgres server vnet-rule create : Create a virtual network rule to allows access to a
            % PostgreSQL server.
            this.BaseCmd = 'az postgres server vnet-rule create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az postgres server vnet-rule create -g testgroup -s testsvr -n vnetRuleName --subnet /subscr')
            fprintf('%s\n', '        iptions/{SubID}/resourceGroups/{ResourceGroup}/providers/Microsoft.Network/virtualNetworks/v')
            fprintf('%s\n', '        netName/subnets/subnetName')
            ...
            fprintf('%s\n', '        az postgres server vnet-rule create -g testgroup -s testsvr -n vnetRuleName --subnet')
            fprintf('%s\n', '        subnetName --vnet-name vnetName')
            fprintf('%s\n', 'For more specific examples, use: az find "az postgres server vnet-rule create"')
