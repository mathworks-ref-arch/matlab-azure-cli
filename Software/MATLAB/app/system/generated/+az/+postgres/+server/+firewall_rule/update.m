classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            ...
            this.BaseCmd = 'az postgres server firewall-rule update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az postgres server firewall-rule update -g testgroup -s testsvr -n allowiprange --start-ip-')
            fprintf('%s\n', '        address 107.46.14.1')
            fprintf('%s\n', '    Update a firewall rule''s start and end IP address.')
            fprintf('%s\n', '        az postgres server firewall-rule update -g testgroup -s testsvr -n allowiprange --start-ip-')
            fprintf('%s\n', '        address 107.46.14.2 --end-ip-address 107.46.14.218')
            fprintf('%s\n', 'For more specific examples, use: az find "az postgres server firewall-rule update"')
