classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az postgres server vnet-rule update : Update a virtual network rule.
            this.BaseCmd = 'az postgres server vnet-rule update ';
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az postgres server vnet-rule update"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az postgres server vnet-rule update : Update a virtual network rule.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --subnet          [Required] : Name or ID of the subnet that allows access to an Azure Postgres')
            fprintf('%s\n', '                                   Server. If subnet name is provided, --vnet-name must be provided.')
            fprintf('%s\n', '    --ignore-missing-endpoint -i : Create vnet rule before virtual network has vnet service endpoint')
            ...
            fprintf('%s\n', '                                   arguments.')
            fprintf('%s\n', '    --name -n                    : The name of the vnet rule.')
            fprintf('%s\n', '    --resource-group -g          : Name of resource group. You can configure the default group using')
            fprintf('%s\n', '                                   `az configure --defaults group=<name>`.')
            fprintf('%s\n', '    --server-name -s             : Name of the server. The name can contain only lowercase letters,')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az postgres server vnet-rule update"')
