classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            ...
            this.BaseCmd = 'az postgres server create ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     location=<location>`.')
            fprintf('%s\n', '    --minimal-tls-version          : Set the minimal TLS version for connections to server when SSL')
            fprintf('%s\n', '                                     is enabled. Default is TLSEnforcementDisabled.  Allowed values:')
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Create a PostgreSQL server in North Europe with sku GP_Gen5_2 (General Purpose, Gen 5 hardware,')
            fprintf('%s\n', '    2 vCores).')
            ...
            fprintf('%s\n', '        az postgres server create -l northeurope -g testgroup -n testsvr -u username -p password \')
            fprintf('%s\n', '            --sku-name B_Gen5_1 --ssl-enforcement Enabled --minimal-tls-version TLS1_0 --public-')
            fprintf('%s\n', '        network-access Disabled \')
            fprintf('%s\n', '            --backup-retention 10 --geo-redundant-backup Enabled --storage-size 51200 \')
            fprintf('%s\n', '            --tags "key=value" --version 11.0')
            fprintf('%s\n', 'For more specific examples, use: az find "az postgres server create"')
