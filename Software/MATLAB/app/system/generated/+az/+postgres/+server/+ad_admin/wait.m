classdef wait < azure.AzureBase
    ...
    methods
        function this = wait()
            % az postgres server ad-admin wait : Place the CLI in a waiting state until a condition of the
            % PostgreSQL server Active Directory Administrator is met.
            % Command group 'postgres server ad-admin' is in preview. It may be changed/removed in a
            % future release.
            this.BaseCmd = 'az postgres server ad-admin wait ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az postgres server ad-admin wait"')
