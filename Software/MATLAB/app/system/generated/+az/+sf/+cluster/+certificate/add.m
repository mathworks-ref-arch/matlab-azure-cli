classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            ...
            this.BaseCmd = 'az sf cluster certificate add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az sf cluster certificate add --cluster-name cluster1 --resource-group group-name --secret-')
            fprintf('%s\n', '        identifier ''https://{KeyVault}.vault.azure.net/secrets/{Secret}'' --vault-name MyVault')
            fprintf('%s\n', 'For more specific examples, use: az find "az sf cluster certificate add"')
