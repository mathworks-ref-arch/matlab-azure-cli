classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            ...
            % Command group 'bot slack' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az bot slack create ';
        end
        ...
        function this = verification_token(this, varargin)
            % The verification token from Slack.
            this.Options = [this.Options, '--verification-token', varargin{:}];
        end
        ...
        function this = landing_page_url(this, varargin)
            % The landing page url to redirect to after login.
            this.Options = [this.Options, '--landing-page-url', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --client-id          [Required] : The client ID from Slack.')
            fprintf('%s\n', '    --client-secret      [Required] : The client secret from Slack.')
            fprintf('%s\n', '    --name -n            [Required] : The resource name of the bot. Bot name must be between 4 and')
            fprintf('%s\n', '                                      42 characters in length. Bot name can only have the following')
            fprintf('%s\n', '                                      characters -, a - z, A - Z, 0 - 9, and _.')
            fprintf('%s\n', '    --resource-group -g  [Required] : Name of resource group. You can configure the default group')
            fprintf('%s\n', '                                      using `az configure --defaults group=<name>`.')
            fprintf('%s\n', '    --verification-token [Required] : The verification token from Slack.')
            fprintf('%s\n', '    --add-disabled                  : Add the channel in a disabled state.  Allowed values: false,')
            fprintf('%s\n', '                                      true.')
            fprintf('%s\n', '    --landing-page-url              : The landing page url to redirect to after login.')
            fprintf('%s\n', 'Global Arguments')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az bot slack create"')
