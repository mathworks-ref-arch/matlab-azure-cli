classdef attach < azure.AzureBase
    ...
    methods
        function this = attach()
            ...
            % or resizing.
            this.BaseCmd = 'az vm unmanaged-disk attach ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --caching                      : Disk caching policy.  Allowed values: None, ReadOnly,')
            fprintf('%s\n', '                                     ReadWrite.')
            fprintf('%s\n', '    --lun                          : 0-based logical unit number (LUN). Max value depends on the')
            fprintf('%s\n', '                                     Virtual Machine size.')
            fprintf('%s\n', '    --name -n                      : The data disk name.')
            fprintf('%s\n', '    --new                          : Create a new disk.')
            fprintf('%s\n', '    --size-gb -z                   : Size in GB. Max size: 4095 GB (certain preview disks can be')
            fprintf('%s\n', '                                     larger).  Default: 1023.')
            fprintf('%s\n', '    --vhd-uri                      : Virtual hard disk URI. For example:')
            fprintf('%s\n', '                                     https://mystorage.blob.core.windows.net/vhds/d1.vhd.')
            fprintf('%s\n', 'Global Arguments')
            ...
            fprintf('%s\n', '        az vm unmanaged-disk attach --name MyDataDisk --new --resource-group MyResourceGroup --size-')
            fprintf('%s\n', '        gb 50 --vm-name MyVm')
            fprintf('%s\n', 'For more specific examples, use: az find "az vm unmanaged-disk attach"')
