classdef resize < azure.AzureBase
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az vm resize : Update a VM''s size.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --size   [Required] : The VM size.  Values from: az vm list-vm-resize-options.')
            ...
            fprintf('%s\n', '        az vm resize --size Standard_DS3_v2 --ids $(az vm list -g MyResourceGroup --query "[].id" -o')
            fprintf('%s\n', '        tsv)')
            fprintf('%s\n', 'For more specific examples, use: az find "az vm resize"')
