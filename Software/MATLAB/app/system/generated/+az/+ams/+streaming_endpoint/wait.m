classdef wait < azure.AzureBase
    ...
    methods
        function this = wait()
            % az ams streaming-endpoint wait : Place the CLI in a waiting state until a condition of the
            % streaming endpoint is met.
            this.BaseCmd = 'az ams streaming-endpoint wait ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az ams streaming-endpoint wait -g MyResourceGroup -a MyAmsAccount -n MyStreamingEndpoint')
            fprintf('%s\n', '        --created')
            fprintf('%s\n', 'For more specific examples, use: az find "az ams streaming-endpoint wait"')
