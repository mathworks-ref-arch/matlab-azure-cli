classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az ams account-filter create : Create an account filter.
            this.BaseCmd = 'az ams account-filter create ';
        end
        ...
        function this = tracks(this, varargin)
            % The JSON representing the track selections. Use @{file} to load from a file. For further information about the JSON structure please refer to swagger documentation on https://docs.microsoft .com/rest/api/media/accountfilters/createorupdate#filtertrackse lection. Presentation Time Range Arguments
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --name -n           [Required] : The name of the account filter.')
            ...
            fprintf('%s\n', '        az ams account-filter create -a amsAccount -g resourceGroup -n filterName --force-end-')
            fprintf('%s\n', '        timestamp=False --end-timestamp 200000 --start-timestamp 100000 --live-backoff-duration 60')
            fprintf('%s\n', '        --presentation-window-duration 600000 --timescale 1000 --first-quality 720 --tracks')
            fprintf('%s\n', '        @C:\tracks.json')
            fprintf('%s\n', 'For more specific examples, use: az find "az ams account-filter create"')
