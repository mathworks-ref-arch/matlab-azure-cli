classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az ams account-filter update : Update the details of an account filter.
            this.BaseCmd = 'az ams account-filter update ';
        end
        ...
        function this = tracks(this, varargin)
            % The JSON representing the track selections. Use @{file} to load from a file. For further information about the JSON structure please refer to swagger documentation on https://docs.microsoft .com/rest/api/media/accountfilters/createorupdate#filtertrackse lection.
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az ams account-filter update"
        ...
        function this = set(this, varargin)
            % Update an object by specifying a property path and value to set.  Example: --set property1.property2=<value>. Presentation Time Range Arguments
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                     .com/rest/api/media/accountfilters/createorupdate#filtertrackse')
            fprintf('%s\n', '                                     lection.')
            ...
            fprintf('%s\n', '                                     playlist. The unit for this property is timescale (see below).')
            fprintf('%s\n', '                                     For example, set presentationWindowDuration=1200000000 to apply')
            fprintf('%s\n', '                                     a two-minute sliding window. Media within 2 minutes of the live')
            fprintf('%s\n', '                                     edge will be included in the playlist. If a fragment straddles')
            fprintf('%s\n', '                                     the boundary, the entire fragment will be included in the')
            fprintf('%s\n', '                                     playlist. The minimum presentation window duration is 60')
            fprintf('%s\n', '                                     seconds.')
            ...
            fprintf('%s\n', '                                     Range, specified as the number of increments in one second.')
            fprintf('%s\n', '                                     Default is 10000000 - ten million increments in one second,')
            fprintf('%s\n', '                                     where each increment would be 100 nanoseconds long. For')
            ...
            fprintf('%s\n', '                                     ''Resource Id'' arguments.')
            fprintf('%s\n', '    --name -n                      : The name of the account filter.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az ams account-filter update"')
