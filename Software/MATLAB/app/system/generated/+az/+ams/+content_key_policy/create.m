classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            % az ams content-key-policy create : Create a new content key policy.
            this.BaseCmd = 'az ams content-key-policy create ';
        end
        ...
        function this = description(this, varargin)
            % The content key policy description. Clear Key Configuration (AES Encryption) Arguments
        ...
        function this = play_ready_template(this, varargin)
            % JSON PlayReady license template. Use @{file} to load from a file. Token Restriction Arguments
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --name -n                 [Required] : The content key policy name.')
            ...
            fprintf('%s\n', '        az ams content-key-policy create -a amsAccount -g resourceGroup -n contentKeyPolicyName')
            fprintf('%s\n', '        --policy-option-name policyOptionName --open-restriction --ask "ask-32-chars-hex-string"')
            fprintf('%s\n', '        --fair-play-pfx pfxPath --fair-play-pfx-password "pfxPassword" --rental-and-lease-key-type')
            fprintf('%s\n', '        PersistentUnlimited --rental-duration 5000')
            fprintf('%s\n', 'For more specific examples, use: az find "az ams content-key-policy create"')
