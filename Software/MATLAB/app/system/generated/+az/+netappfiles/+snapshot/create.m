classdef create < azure.AzureBase
    ...
    methods
        function this = create()
            ...
            % release.
            this.BaseCmd = 'az netappfiles snapshot create ';
        end
        ...
        function this = file_system_id(this, varargin)
            % The uuid of the volume.
            this.Options = [this.Options, '--file-system-id', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --name --snapshot-name -n -s [Required] : The name of the ANF snapshot.')
            fprintf('%s\n', '    --pool-name -p               [Required] : The name of the ANF pool.')
            fprintf('%s\n', '    --resource-group -g          [Required] : Name of resource group. You can configure the default')
            fprintf('%s\n', '                                              group using `az configure --defaults group=<name>`.')
            fprintf('%s\n', '    --volume-name -v             [Required] : The name of the ANF volume.')
            fprintf('%s\n', '    --file-system-id                        : The uuid of the volume.')
            fprintf('%s\n', 'Global Arguments')
            ...
            fprintf('%s\n', '        az netappfiles snapshot create -g mygroup --account-name myaccname --pool-name mypoolname')
            fprintf('%s\n', '        --volume-name myvolname --name mysnapname -l eastus --file-system-id')
            fprintf('%s\n', '        13641da9-c0e9-4b97-84fc-4f8014a93848')
            fprintf('%s\n', 'For more specific examples, use: az find "az netappfiles snapshot create"')
