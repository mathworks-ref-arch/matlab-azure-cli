classdef suspend < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = suspend()
            % az netappfiles volume replication suspend : Suspend/break a volume replication for the specified
            % destination volume. The replication process is suspended until resumed or deleted.
            % Command group 'netappfiles' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az netappfiles volume replication suspend ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az netappfiles volume replication suspend -g mygroup --account-name myaccname --pool-name')
            fprintf('%s\n', '        mypoolname --name mydestinationvolname')
            fprintf('%s\n', 'For more specific examples, use: az find "az netappfiles volume replication suspend"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class suspend 
