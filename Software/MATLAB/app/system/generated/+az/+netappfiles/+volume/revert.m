classdef revert < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = revert()
            % az netappfiles volume revert : Revert a volume to one of its snapshots.
            % Revert a volume to the snapshot specified in the body.
            % Command group 'netappfiles' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az netappfiles volume revert ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --snapshot-id -s           [Required] : Snapshot_Id of the snapshot. UUID v4 used to identify')
            ...
            fprintf('%s\n', '        az netappfiles volume revert -g mygroup --account-name myaccname --pool-name mypoolname')
            fprintf('%s\n', '        --name myvolname --snapshot-id 9760acf5-4638-11e7-9bdb-020073ca3333')
            fprintf('%s\n', 'For more specific examples, use: az find "az netappfiles volume revert"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class revert 
