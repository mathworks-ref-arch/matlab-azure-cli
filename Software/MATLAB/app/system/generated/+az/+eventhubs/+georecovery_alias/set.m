classdef set < azure.AzureBase
    ...
    methods
        function this = set()
            % az eventhubs georecovery-alias set : Sets a Geo-Disaster Recovery Configuration Alias for the
            % give Namespace.
            this.BaseCmd = 'az eventhubs georecovery-alias set ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    give Namespace.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --partner-namespace [Required] : Name (if within the same resource group) or ARM Id of the')
            fprintf('%s\n', '                                     Primary/Secondary eventhub namespace name, which is part of GEO')
            fprintf('%s\n', '                                     DR pairing.')
            fprintf('%s\n', '    --alternate-name               : Alternate Name for the Alias, when the Namespace name and Alias')
            fprintf('%s\n', '                                     name are same.')
            fprintf('%s\n', 'Resource Id Arguments')
            fprintf('%s\n', '    --alias -a                     : Name of Geo-Disaster Recovery Configuration Alias.')
            ...
            fprintf('%s\n', '        az eventhubs georecovery-alias set --resource-group myresourcegroup --namespace-name')
            fprintf('%s\n', '        primarynamespace --alias myaliasname --partner-namespace resourcearmid')
            fprintf('%s\n', '    Sets a Geo-Disaster Recovery Configuration Alias for the give Namespace (autogenerated)')
            fprintf('%s\n', '        az eventhubs georecovery-alias set --alias myaliasname --namespace-name primarynamespace')
            fprintf('%s\n', '        --partner-namespace resourcearmid --resource-group myresourcegroup --subscription')
            fprintf('%s\n', '        MySubscription')
            fprintf('%s\n', 'For more specific examples, use: az find "az eventhubs georecovery-alias set"')
