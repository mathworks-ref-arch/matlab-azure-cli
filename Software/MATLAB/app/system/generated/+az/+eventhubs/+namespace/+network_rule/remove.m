classdef remove < azure.AzureBase
    ...
    methods
        function this = remove()
            ...
            this.BaseCmd = 'az eventhubs namespace network-rule remove ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az eventhubs namespace network-rule remove --resource-group myresourcegroup --namespace-name')
            fprintf('%s\n', '        mynamespace --subnet /subscriptions/{subscriptionid}/resourcegroups/{resourcegroupname}/prov')
            fprintf('%s\n', '        iders/Microsoft.Network/virtualNetworks/{virtualnetworkname}/subnets/{subnetname}')
            fprintf('%s\n', '    remove IP rule from NetworkruleSet for a namespace')
            fprintf('%s\n', '        az eventhubs namespace network-rule remove --resource-group myresourcegroup --namespace-name')
            fprintf('%s\n', '        mynamespace --ip-address 10.6.0.0/24')
            fprintf('%s\n', 'For more specific examples, use: az find "az eventhubs namespace network-rule remove"')
