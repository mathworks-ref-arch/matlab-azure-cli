classdef remove < azure.AzureBase
    ...
    methods
        function this = remove()
            % az functionapp config access-restriction remove : Removes an Access Restriction from the
            % functionapp.
            % Command group 'functionapp config access-restriction' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az functionapp config access-restriction remove ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az functionapp config access-restriction remove -g ResourceGroup -n AppName --rule-name')
            fprintf('%s\n', '        developers')
            fprintf('%s\n', '    Remove Access Restriction named internal_agents from the scm site.')
            fprintf('%s\n', '        az functionapp config access-restriction remove -g ResourceGroup -n AppName --rule-name')
            fprintf('%s\n', '        internal_agents --scm-site true')
            fprintf('%s\n', 'For more specific examples, use: az find "az functionapp config access-restriction remove"')
