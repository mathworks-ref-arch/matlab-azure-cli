classdef list < azure.AzureBase
    ...
    methods
        function this = list()
            % az consumption reservation detail list : List the details of a reservation by order id or
            % reservation id.
            % Command group 'consumption' is in preview. It may be changed/removed in a future
            % release.
            this.BaseCmd = 'az consumption reservation detail list ';
        end
        ...
        function this = verbose(this, varargin)
            % Increase logging verbosity. Use --debug for full debug logs. For more specific examples, use: az find "az consumption reservation detail list"
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        release.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --end-date -e          [Required] : End date (YYYY-MM-DD in UTC). Only needed for daily grain')
            fprintf('%s\n', '                                        and if specified, also requires --start-date.')
            fprintf('%s\n', '    --reservation-order-id [Required] : Reservation order id.')
            fprintf('%s\n', '    --start-date -s        [Required] : Start date (YYYY-MM-DD in UTC). Only needed for daily grain')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az consumption reservation detail list"')
