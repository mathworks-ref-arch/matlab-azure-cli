classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            % az servicebus namespace network-rule add : Add a network rule for a namespace.
            this.BaseCmd = 'az servicebus namespace network-rule add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az servicebus namespace network-rule add --resource-group myresourcegroup --namespace-name')
            fprintf('%s\n', '        mynamespace --subnet {subnetId} --ignore-missing-endpoint True')
            fprintf('%s\n', '    add a IP rule in NetworkruleSet for a namespace')
            fprintf('%s\n', '        az servicebus namespace network-rule add --resource-group myresourcegroup --namespace-name')
            fprintf('%s\n', '        mynamespace --ip-address 10.0.0.0/24 --action Allow')
            fprintf('%s\n', 'For more specific examples, use: az find "az servicebus namespace network-rule add"')
