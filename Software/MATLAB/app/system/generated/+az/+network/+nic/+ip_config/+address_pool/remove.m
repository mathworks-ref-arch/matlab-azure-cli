classdef remove < azure.AzureBase
    ...
    methods
        function this = remove()
            ...
            this.BaseCmd = 'az network nic ip-config address-pool remove ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az network nic ip-config address-pool remove --address-pool MyAddressPool --ip-config-name')
            fprintf('%s\n', '        MyIpConfig --lb-name MyLb --nic-name MyNic --resource-group MyResourceGroup')
            fprintf('%s\n', 'For more specific examples, use: az find "az network nic ip-config address-pool remove"')
