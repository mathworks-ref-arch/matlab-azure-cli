classdef set_record < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = set_record()
            % az network private-dns record-set cname set-record : Set the value of a CNAME record.
            this.BaseCmd = 'az network private-dns record-set cname set-record ';
        end
        function this = cname(this, varargin)
            % Canonical name.
            this.Options = [this.Options, '--cname', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az network private-dns record-set cname set-record : Set the value of a CNAME record.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --cname -c           [Required] : Canonical name.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-dns record-set cname set-')
            fprintf('%s\n', 'record"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class set_record 
