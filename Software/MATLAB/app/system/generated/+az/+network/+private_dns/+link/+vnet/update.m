classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az network private-dns link vnet update : Update a virtual network link's properties. Does not
            % modify virtual network within the link.
            this.BaseCmd = 'az network private-dns link vnet update ';
        end
        ...
        function this = if_match(this, varargin)
            % The ETag of the virtual network link to the Private DNS zone. Omit this value to always overwrite the current virtual network link. Specify the last-seen ETag value to prevent accidentally overwritting any concurrent changes.
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    modify virtual network within the link.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --name -n           [Required] : The name of the virtual network link to the specified Private')
            fprintf('%s\n', '                                     DNS zone.')
            fprintf('%s\n', '    --resource-group -g [Required] : Name of resource group. You can configure the default group')
            fprintf('%s\n', '                                     using `az configure --defaults group=<name>`.')
            fprintf('%s\n', '    --zone-name -z      [Required] : The name of the Private DNS zone.')
            fprintf('%s\n', '    --if-match                     : The ETag of the virtual network link to the Private DNS zone.')
            fprintf('%s\n', '        Omit this value to always overwrite the current virtual network link. Specify the last-seen')
            fprintf('%s\n', '        ETag value to prevent accidentally overwritting any concurrent changes.')
            fprintf('%s\n', '    --no-wait                      : Do not wait for the long-running operation to finish.')
            fprintf('%s\n', '    --registration-enabled -e      : Specify if the link is registration enabled.  Allowed values:')
            fprintf('%s\n', '                                     false, true.')
            ...
            fprintf('%s\n', '        az network private-dns link vnet update -g MyResourceGroup -n MyLinkName -z www.mysite.com')
            fprintf('%s\n', '        -e True')
            fprintf('%s\n', '    Update a virtual network link''s properties (autogenerated)')
            fprintf('%s\n', '        az network private-dns link vnet update --name MyLinkName --registration-enabled true')
            fprintf('%s\n', '        --resource-group MyResourceGroup --tags CostCenter=Marketing --zone-name www.mysite.com')
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-dns link vnet update"')
