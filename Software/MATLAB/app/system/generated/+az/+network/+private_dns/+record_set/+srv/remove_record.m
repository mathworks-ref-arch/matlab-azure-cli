classdef remove_record < azure.AzureBase
    ...
    methods
        function this = remove_record()
            % az network private-dns record-set srv remove-record : Remove an SRV record from its record set.
            ...
            this.BaseCmd = 'az network private-dns record-set srv remove-record ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-dns record-set srv remove-')
            fprintf('%s\n', 'record"')
