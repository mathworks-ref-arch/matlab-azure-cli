classdef remove_record < azure.AzureBase
    ...
    methods
        function this = remove_record()
            % az network private-dns record-set aaaa remove-record : Remove AAAA record from its record set.
            ...
            this.BaseCmd = 'az network private-dns record-set aaaa remove-record ';
        end
        function this = ipv6_address(this, varargin)
            % IPV6 address in string notation.
            this.Options = [this.Options, '--ipv6-address', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        empty record set, include --keep-empty-record-set.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --ipv6-address -a    [Required] : IPV6 address in string notation.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-dns record-set aaaa remove-')
            fprintf('%s\n', 'record"')
