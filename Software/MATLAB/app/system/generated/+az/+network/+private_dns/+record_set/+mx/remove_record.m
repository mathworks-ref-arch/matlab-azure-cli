classdef remove_record < azure.AzureBase
    ...
    methods
        function this = remove_record()
            % az network private-dns record-set mx remove-record : Remove an MX record from its record set.
            ...
            this.BaseCmd = 'az network private-dns record-set mx remove-record ';
        end
        function this = exchange(this, varargin)
            % Exchange metric.
            this.Options = [this.Options, '--exchange', varargin{:}];
        end

        function this = preference(this, varargin)
            % Preference metric.
            this.Options = [this.Options, '--preference', varargin{:}];
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        empty record set, include --keep-empty-record-set.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --exchange -e        [Required] : Exchange metric.')
            fprintf('%s\n', '    --preference -p      [Required] : Preference metric.')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-dns record-set mx remove-')
            fprintf('%s\n', 'record"')
