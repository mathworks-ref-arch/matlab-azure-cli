classdef show < azure.AzureBase
    ...
    methods
        function this = show()
            % az network private-endpoint-connection show : Show a private endpoint connection.
            this.BaseCmd = 'az network private-endpoint-connection show ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '        az network private-endpoint-connection show -g MyResourceGroup -n MyPrivateEndpoint')
            fprintf('%s\n', '        --resource-name MySA --type Microsoft.Storage/storageAccounts')
            fprintf('%s\n', '    Show a private endpoint connection for a keyvault.')
            fprintf('%s\n', '        az network private-endpoint-connection show -g MyResourceGroup -n MyPrivateEndpoint')
            fprintf('%s\n', '        --resource-name MyKV --type Microsoft.Keyvault/vaults')
            fprintf('%s\n', '    Show a private endpoint connection for an ACR.')
            fprintf('%s\n', '        az network private-endpoint-connection show --id /subscriptions/00000000-0000-0000-0000-0000')
            fprintf('%s\n', '        00000000/resourceGroups/clitest.rg000001/providers/Microsoft.ContainerRegistry/registries/te')
            fprintf('%s\n', '        streg000002/privateEndpointConnections/testreg000002.6e6bf72bc59d41cc89c698d4cc5ee79d')
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-endpoint-connection show"')
