classdef approve < azure.AzureBase
    ...
    methods
        function this = approve()
            % az network private-endpoint-connection approve : Approve a private endpoint connection.
            this.BaseCmd = 'az network private-endpoint-connection approve ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az network private-endpoint-connection approve : Approve a private endpoint connection.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --description -d    : Comments for the approval.')
            ...
            fprintf('%s\n', '        az network private-endpoint-connection approve -g MyResourceGroup -n MyPrivateEndpoint')
            fprintf('%s\n', '        --resource-name MySA --type Microsoft.Storage/storageAccounts --description "Approved"')
            fprintf('%s\n', '    Approve a private endpoint connection for a keyvault.')
            fprintf('%s\n', '        az network private-endpoint-connection approve -g MyResourceGroup -n MyPrivateEndpoint')
            fprintf('%s\n', '        --resource-name MyKV --type Microsoft.Keyvault/vaults --description "Approved"')
            fprintf('%s\n', '    Approve a private endpoint connection for an ACR.')
            fprintf('%s\n', '        az network private-endpoint-connection approve --id /subscriptions/00000000-0000-0000-0000-0')
            fprintf('%s\n', '        00000000000/resourceGroups/clitest.rg000001/providers/Microsoft.ContainerRegistry/registries')
            fprintf('%s\n', '        /testreg000002/privateEndpointConnections/testreg000002.6e6bf72bc59d41cc89c698d4cc5ee79d')
            fprintf('%s\n', '        --description "Approved"')
            fprintf('%s\n', 'For more specific examples, use: az find "az network private-endpoint-connection approve"')
