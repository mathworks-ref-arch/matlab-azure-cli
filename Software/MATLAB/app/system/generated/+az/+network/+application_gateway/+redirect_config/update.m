classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            ...
            this.BaseCmd = 'az network application-gateway redirect-config update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    az network application-gateway redirect-config update : Update a redirect configuration.')
            fprintf('%s\n', 'Arguments')
            fprintf('%s\n', '    --include-path         : Include path in the redirected url.  Allowed values: false, true.')
            fprintf('%s\n', '    --include-query-string : Include query string in the redirected url.  Allowed values: false,')
            fprintf('%s\n', '                             true.')
            ...
            fprintf('%s\n', '    --type -t              : HTTP redirection type.  Allowed values: Found, Permanent, SeeOther,')
            fprintf('%s\n', '                             Temporary.')
            ...
            fprintf('%s\n', '    --name -n              : The name of the redirect configuration.')
            ...
            fprintf('%s\n', '        az network application-gateway redirect-config update -g MyResourceGroup --gateway-name')
            fprintf('%s\n', '        MyAppGateway \')
            fprintf('%s\n', '            -n MyRedirectConfig --type Permanent --target-listener MyNewBackendListener')
            fprintf('%s\n', '    Update a redirect configuration. (autogenerated)')
            fprintf('%s\n', '        az network application-gateway redirect-config update --gateway-name MyAppGateway --include-')
            fprintf('%s\n', '        path true --include-query-string true --name MyRedirectConfig --resource-group')
            fprintf('%s\n', '        MyResourceGroup --target-listener MyNewBackendListener --type Permanent')
