classdef remove < azure.AzureBase
    ...
    methods
        function this = remove()
            % az network application-gateway waf-policy managed-rule rule-set remove : Remove a managed rule
            % set by rule set group name if rule_group_name is specified. Otherwise, remove all rule set.
            this.BaseCmd = 'az network application-gateway waf-policy managed-rule rule-set remove ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Remove a managed rule set by rule set group name if rule_group_name is specified. Otherwise,')
            fprintf('%s\n', '    remove all rule set.')
            fprintf('%s\n', '        az network application-gateway waf-policy managed-rule rule-set remove --policy-name')
            fprintf('%s\n', '        MyPolicy --resource-group MyResourceGroup --type Permanent --version IPv4')
