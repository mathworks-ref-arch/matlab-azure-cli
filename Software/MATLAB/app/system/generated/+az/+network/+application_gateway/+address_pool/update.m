classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            ...
            this.BaseCmd = 'az network application-gateway address-pool update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --servers           : Space-separated list of IP addresses or DNS names corresponding to backend')
            fprintf('%s\n', '                          servers.')
            ...
            fprintf('%s\n', '    --name -n           : The name of the backend address pool.')
            ...
            fprintf('%s\n', '        az network application-gateway address-pool update -g MyResourceGroup --gateway-name')
            fprintf('%s\n', '        MyAppGateway \ -n MyAddressPool --servers 10.0.0.4 10.0.0.5 10.0.0.6')
            fprintf('%s\n', '    Add to the backend address pool by using backend server IP address.')
            fprintf('%s\n', '        az network application-gateway address-pool update -g MyResourceGroup --gateway-name')
            fprintf('%s\n', '        MyAppGateway -n MyAddressPool \')
            fprintf('%s\n', '            --add backendAddresses "{ "ip_address": "{10.0.0.13}" }"')
            fprintf('%s\n', 'For more specific examples, use: az find "az network application-gateway address-pool')
            fprintf('%s\n', 'update"')
