classdef show_status < azure.AzureBase

    % Copyright 2020, The MathWorks Inc.

    methods
        function this = show_status()
            % az network watcher packet-capture show-status : Show the status of a packet capture session.
            this.BaseCmd = 'az network watcher packet-capture show-status ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                               location=<location>`.')
            fprintf('%s\n', '    --name -n     [Required] : The name given to the packet capture session.')
            fprintf('%s\n', 'Global Arguments')
            ...
            fprintf('%s\n', 'For more specific examples, use: az find "az network watcher packet-capture show-status"')
            fprintf('%s\n', 'Please let us know how we are doing: https://aka.ms/azureclihats')
        end
    end
end % End of class show_status 
