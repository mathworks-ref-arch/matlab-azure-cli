classdef wait < azure.AzureBase
    ...
    methods
        function this = wait()
            % az network express-route wait : Place the CLI in a waiting state until a condition of the
            % ExpressRoute is met.
            this.BaseCmd = 'az network express-route wait ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', 'Examples')
            fprintf('%s\n', '    Pause executing next line of CLI script until the ExpressRoute circuit is successfully')
            fprintf('%s\n', '    provisioned.')
            fprintf('%s\n', '        az network express-route wait -n MyCircuit -g MyResourceGroup --created')
            fprintf('%s\n', 'For more specific examples, use: az find "az network express-route wait"')
