classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            ...
            this.BaseCmd = 'az network lb inbound-nat-pool update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '                                  unexpected connection termination. Only used when protocol is set')
            fprintf('%s\n', '                                  to TCP.  Allowed values: false, true.')
            fprintf('%s\n', '    --floating-ip               : Enable floating IP.  Allowed values: false, true.')
            fprintf('%s\n', '    --frontend-ip-name          : The name of the frontend IP configuration.')
            fprintf('%s\n', '    --frontend-port-range-end   : Port number.')
            fprintf('%s\n', '    --frontend-port-range-start : Port number.')
            ...
            fprintf('%s\n', '    --name -n                   : The name of the inbound NAT pool.')
            ...
            fprintf('%s\n', '        az network lb inbound-nat-pool update --backend-port 8080 --enable-tcp-reset true')
            fprintf('%s\n', '        --frontend-port-range-end 89 --frontend-port-range-start 80 --lb-name MyLb --name MyNatPool')
            fprintf('%s\n', '        --resource-group MyResourceGroup')
            fprintf('%s\n', '    Update an inbound NAT address pool. (autogenerated)')
            fprintf('%s\n', '        az network lb inbound-nat-pool update --enable-tcp-reset true --lb-name MyLb --name')
            fprintf('%s\n', '        MyNatPool --protocol Udp --resource-group MyResourceGroup')
            fprintf('%s\n', '    Update an inbound NAT address pool. (autogenerated)')
            fprintf('%s\n', '        az network lb inbound-nat-pool update --backend-port 8080 --frontend-port-range-end 89')
            fprintf('%s\n', '        --frontend-port-range-start 80 --lb-name MyLb --name MyNatPool --protocol Udp --resource-')
            fprintf('%s\n', '        group MyResourceGroup')
            fprintf('%s\n', 'For more specific examples, use: az find "az network lb inbound-nat-pool update"')
