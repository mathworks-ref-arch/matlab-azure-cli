classdef add < azure.AzureBase
    ...
    methods
        function this = add()
            % az network lb address-pool address add : Add one backend address into the load balance backend
            % address pool.
            % Command group 'network lb address-pool address' is in preview. It may be
            % changed/removed in a future release.
            this.BaseCmd = 'az network lb address-pool address add ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    --name -n           [Required] : Name of the backend address.')
            ...
            fprintf('%s\n', '        az network lb address-pool address add -g MyResourceGroup --lb-name MyLb --pool-name')
            fprintf('%s\n', '        MyAddressPool -n MyAddress --vnet MyVnet --ip-address 10.0.0.1')
            fprintf('%s\n', 'For more specific examples, use: az find "az network lb address-pool address add"')
