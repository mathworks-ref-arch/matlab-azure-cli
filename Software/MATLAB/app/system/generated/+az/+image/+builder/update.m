classdef update < azure.AzureBase
    ...
    methods
        function this = update()
            % az image builder update : Update an image builder template.
            % Updating an image builder templates is currently unsupported. This command can be used in
            % conjunction with --defer to update an image template object within the CLI cache. Without
            % --defer it retrieves the specified image template from the cache and sends a request to
            % Azure to create the image template.
            this.BaseCmd = 'az image builder update ';
        end
    ...
    methods (Static = true)
        function help(~)
            ...
            fprintf('%s\n', '    See "az image builder create / output add / customizer add --help" and "az cache -h" for more')
            fprintf('%s\n', '    information')
            ...
            fprintf('%s\n', '            --scripts {script} --managed-image-destinations image_1=westus --identity myidentity')
            fprintf('%s\n', '        --defer')
            fprintf('%s\n', '        # add customizers and outputs to local cache template object via az image template output /')
            fprintf('%s\n', '        customizer add')
            ...
            fprintf('%s\n', '            --artifact-tags "is_vhd=False"  --managed-image winImage --managed-image-location eastus')
            fprintf('%s\n', '        --defer')
            fprintf('%s\n', '        # send template create request to azure to create template resource')
            fprintf('%s\n', '        az image builder update -n mytemplate -g my-group')
            fprintf('%s\n', 'For more specific examples, use: az find "az image builder update"')
